...
class FusionLidarImageOverlay:
    def __init__(self):
        ...
        self.conf_threshold = rospy.get_param("~conf_threshold", 0.5) #0.5
        ...
        self.downsample_rate = rospy.get_param("~downsample_rate", 4) #4
        ...
        self.max_depth = rospy.get_param("~max_depth", 100.0)
    ...
    def apply_sigma_filter(self, points_3d, distances, std_multiplier=1.5):
        ...
        # lower_bound = distance - (std_multiplier * std_dist)
        # upper_bound = distance + (std_multiplier * std_dist)
        ...
        try:
            ...
            for det in target_boxes:
                ...
                if len(filtered_2d) > 0:
                    ...
                    pixel_x = filtered_2d[:,0]

                    weights = 1.0 / (np.abs(pixel_x-center_x)+1)

                    weighted_distance = np.sum(filtered_dist*weights)/np.sum(weights)

                    object_distance = weighted_distance
                    
                    rospy.loginfo(f"Class {class_id}: Mean distance = {object_distance:.2f}")
            ...
            for det in target_boxes:
                ...
                # if 'distance' in det:
                #     label = f"Class {class_id}: {confidence:.2f} (Distance: {distance:.2f})m"
                # else:
                #     label = f"Class {class_id}: {confidence:.2f}"

                if 'distance' in det:
                    label = f"{confidence:.2f} (Distance: {distance:.2f})m"
                else:
